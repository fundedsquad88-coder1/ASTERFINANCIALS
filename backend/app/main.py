...
def admin_blockchain_status(x_aster_admin_key: Optional[str] = Header(default=None, alias="X-Aster-Admin-Key")):
    expected = os.getenv("ASTER_ADMIN_API_KEY", "")
    ...
    return {"ok": True, "bep20": {"configured": bool(os.getenv("ASTER_BSC_RPC_URL")), "confirmations": max(1, int(os.getenv("ASTER_BSC_CONFIRMATIONS", "12")))}, "trc20": {"configured": bool(os.getenv("ASTER_TRON_API_URL") or os.getenv("ASTER_TRON_API_KEY")), "confirmations": max(1, int(os.getenv("ASTER_TRON_CONFIRMATIONS", "1")))}}
...
def admin_blockchain_scan(x_aster_admin_key: Optional[str] = Header(default=None, alias="X-Aster-Admin-Key"), dbs: Session = Depends(db)):
    ...
    try:
        ...
        return {"ok": True, "result": scan_once(dbs)}
    except Exception as exc:
        dbs.rollback()
        raise HTTPException(502, f"Blockchain scan failed: {type(exc).__name__}")

@app.get("/v1/autoinvest/{autoinvest_id}/valuation")
def get_strategy_valuation(autoinvest_id: int, user: User = Depends(current_user), dbs: Session = Depends(db)):
    row = dbs.scalar(select(AutoInvest).where(AutoInvest.id == autoinvest_id, AutoInvest.user_id == user.id))
    if not row:
        raise HTTPException(404, "Auto-Invest not found")
    valuation = dbs.scalar(select(StrategyValuation).where(StrategyValuation.autoinvest_id == row.id).order_by(StrategyValuation.as_of.desc()))
    if not valuation:
        return {"autoinvest_id": row.id, "status": "unpriced"}
    return {"autoinvest_id": row.id, "value": str(valuation.value), "source": valuation.source, "as_of": valuation.as_of, "status": "priced"}

@app.post("/v1/autoinvest/{autoinvest_id}/valuation")
def set_strategy_valuation(autoinvest_id: int, body: ValuationIn, user: User = Depends(current_user), dbs: Session = Depends(db), _: None = Depends(require_csrf)):
    row = dbs.scalar(select(AutoInvest).where(AutoInvest.id == autoinvest_id, AutoInvest.user_id == user.id))
    if not row:
        raise HTTPException(404, "Auto-Invest not found")
    validate_valuation(body.value, body.as_of)
    valuation = StrategyValuation(autoinvest_id=row.id, value=body.value, source=body.source, as_of=utc_datetime(body.as_of))
    dbs.add(valuation)
    dbs.commit()
    return {"autoinvest_id": row.id, "value": str(valuation.value), "source": valuation.source, "as_of": valuation.as_of, "status": "priced"}

@app.post("/v1/autoinvest/{autoinvest_id}/cancel")
def cancel_autoinvest(autoinvest_id: int, user: User = Depends(current_user), dbs: Session = Depends(db), _: None = Depends(require_csrf)):
    row = dbs.scalar(select(AutoInvest).where(AutoInvest.id == autoinvest_id, AutoInvest.user_id == user.id).with_for_update())
    if not row:
        raise HTTPException(404, "Auto-Invest not found")
    if row.status not in {"active", "paused"}:
        raise HTTPException(409, "Strategy cannot be cancelled")
    reference = "AIC-" + secrets.token_hex(6).upper()
    transfer_wallet_balance(dbs, user_id=user.id, amount=row.amount, source="user.invested", destination="user.available", reference=reference)
    wallet_row = dbs.scalar(select(Wallet).where(Wallet.user_id == user.id).with_for_update())
    wallet_row.invested -= row.amount
    wallet_row.available += row.amount
    row.status = "cancelled"
    dbs.add(LedgerEntry(user_id=user.id, kind="autoinvest_cancel", amount=row.amount, reference=reference, status="posted"))
    dbs.add(Notification(user_id=user.id, title="Auto-Invest cancelled", body=f"{row.strategy} has been cancelled."))
    dbs.commit()
    return {"id": row.id, "status": row.status, "released_amount": str(row.amount)}

@app.post("/v1/admin/autoinvest/sync")
def admin_autoinvest_sync_v31(limit: int = 100, x_aster_admin_key: Optional[str] = Header(default=None, alias="X-Aster-Admin-Key"), dbs: Session = Depends(db)):
    expected = os.getenv("ASTER_ADMIN_API_KEY", "")
    if len(expected) < 32 or not x_aster_admin_key or not hmac.compare_digest(expected, x_aster_admin_key):
        raise HTTPException(403, "Admin authorization required")
...
def admin_autoinvest_execute_v31(limit: int = 25, x_aster_admin_key: Optional[str] = Header(default=None, alias="X-Aster-Admin-Key"), dbs: Session = Depends(db)):
    expected = os.getenv("ASTER_ADMIN_API_KEY", "")
    if len(expected) < 32 or not x_aster_admin_key or not hmac.compare_digest(expected, x_aster_admin_key):
        raise HTTPException(403, "Admin authorization required")

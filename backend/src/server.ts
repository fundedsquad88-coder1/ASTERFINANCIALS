...
async function main() {
  ...
  app.post('/api/v1/auth/login', async (request, reply) => {
    ...
    try {
      ...
      return { user: { id: user.id, email: user.email, status: 'ACTIVE', referralCode: 'referralCode' in user ? user.referralCode : null }, token: issueSession(user.id), sandbox: true, persistence: databaseEnabled ? 'database' : 'memory' };
    } catch (error) { request.log.error(error); return reply.code(500).send({ error: 'Unable to sign in' }); }
  });

  app.post('/api/v1/auth/logout', async request => { const token = bearerToken(request); if (token) sessions.delete(crypto.createHash('sha256').update(token).digest('hex')); return { ok: true }; });
  ...
  app.post('/api/v1/withdrawals/request', async (request, reply) => {
    ...
    try {
      const result = await db.$transaction(async tx => {
        ...
        if (!wallet) throw new Error('WALLET_NOT_FOUND'); const amount = decimal(parsed.data.amount);
        ...
        await tx.ledgerEntry.createMany({ data: [{ reference, account: user.id, assetSymbol: 'USDT', direction: 'DEBIT', amount }, { reference, account: 'ASTER_WITHDRAWAL_CLEARING', assetSymbol: 'USDT', direction: 'CREDIT', amount }] });
  ...
  app.get('/api/v1/earn/staking', async (request, reply) => {
    const user = await authenticatedUser(request); if (!user) return reply.code(401).send({ error: 'Authentication required' });
  ...
  app.post('/api/v1/earn/staking/preview', async (request, reply) => {
    ...
    return { sandbox: true, accepted: false, reason: 'Preview only; no funds are locked', asset: 'USDT', amount: parsed.data.amount.toFixed(6), durationDays: parsed.data.durationDays, rateBps: sandboxRateBps, projectedReward: projectedReward.toFixed(6) };
  });
  ...
  app.post('/api/v1/earn/staking/create', async (request, reply) => {
    ...
    try {
      const result = await db.$transaction(async tx => {
        ...
        const stake = await tx.stake.create({ data: { userId: user.id, asset: 'USDT', amount, rateBps, endsAt } });
        await tx.ledgerEntry.createMany({ data: [{ reference: `stake:${stake.id}`, account: user.id, assetSymbol: 'USDT', direction: 'DEBIT', amount }, { reference: `stake:${stake.id}`, account: 'ASTER_STAKING_POOL', assetSymbol: 'USDT', direction: 'CREDIT', amount }] });
        return stake;
      });
      return reply.code(201).send({ id: result.id, asset: result.asset, amount: result.amount.toString(), rateBps: result.rateBps, startedAt: result.startedAt, endsAt: result.endsAt, sandbox: true, message: 'Sandbox stake created. Reward settlement is not automatic yet.' });
    } catch (error) {
      if (error instanceof Error && error.message === 'INSUFFICIENT_BALANCE') return reply.code(400).send({ error: 'Insufficient available USDT balance' });
      if (error instanceof Error && error.message === 'WALLET_NOT_FOUND') return reply.code(404).send({ error: 'USDT wallet not found' });
  ...
  app.get('/api/v1/earn/referral', async (request, reply) => {
    const user = await authenticatedUser(request); if (!user) return reply.code(401).send({ error: 'Authentication required' });
    if (!databaseEnabled) return { referralCode: null, referralCount: 0, rewards: [], sandbox: true };
    const [account, referrals, rewards] = await Promise.all([
      db.user.findUnique({ where: { id: user.id }, select: { referralCode: true } }),
      db.referral.findMany({ where: { referrerId: user.id }, orderBy: { createdAt: 'desc' }, take: 100, include: { referee: { select: { id: true, createdAt: true } } } }),

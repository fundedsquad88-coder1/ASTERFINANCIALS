...
const credentials = new Map<string, { id: string; email: string; passwordHash: string }>();
...
async function main() {
  ...
  app.get('/api/v1/account/:id', async (request, reply) => {
    ...
    const user = [...credentials.values()].find(x => x.id === parsed.data.id);
    if (!user) return reply.code(404).send({ error: 'Account not found' });
    return { id: user.id, email: user.email, status: 'ACTIVE', wallets: [{ asset: 'USDT', available: '0.00', locked: '0.00' }] };
  });
  app.post('/api/v1/auth/register', async (request, reply) => {
    ...
    const id = crypto.randomUUID(); credentials.set(id, { id, email, passwordHash: hashPassword(parsed.data.password) });
    return reply.code(201).send({ id, email, status: 'ACTIVE', sandbox: true });
  });
  app.post('/api/v1/auth/login', async (request, reply) => {
    ...
    return { user: { id: user.id, email: user.email }, token: crypto.randomBytes(32).toString('base64url'), sandbox: true };
  });
  app.post('/api/v1/trades/preview', async (request, reply) => {
    ...
    return { sandbox: true, accepted: false, reason: 'Preview only; no real-money order is submitted', ...parsed.data };
  });

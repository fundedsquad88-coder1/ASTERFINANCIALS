...
async function tronScan(){
  ...
  try{ for(const t of (j.data||[])){ const amount=Number(t.value)/1e6; if(!Number.isFinite(amount)||amount<=0) continue; const tx=t.transaction_id; const idx=String(t.block_timestamp||0); await client.query("INSERT INTO blockchain_transfers(network,tx_hash,transfer_index,block_reference,token_contract,from_address,to_address,amount,confirmations,status) VALUES('TRC-20',$1,$2,$3,$4,$5,$6,$7,$8,'verified') ON CONFLICT(network,tx_hash,transfer_index) DO NOTHING",[tx,idx,String(t.block_timestamp||""),TRC20_USDT_CONTRACT,t.from,t.to,amount,TRON_CONFIRMATIONS]); } } finally { client.release(); }
}

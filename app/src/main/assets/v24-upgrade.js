...
async function forex(){
 try{
  const raw=await new Promise((resolve,reject)=>{const id='fx'+Date.now()+Math.random();window.__asterCallbacks=window.__asterCallbacks||{};window.__asterCallbacks[id]={resolve,reject};if(window.AsterNative)AsterNative.fetch('https://api.frankfurter.dev/v1/latest?base=USD&symbols=EUR,GBP,JPY,AUD,CAD,CHF',id);else fetch('https://api.frankfurter.dev/v1/latest?base=USD&symbols=EUR,GBP,JPY,AUD,CAD,CHF').then(r=>r.text()).then(resolve).catch(reject)});
  const d=JSON.parse(raw), rates=d.rates||{};
  Object.entries(rates).forEach(([ccy,rate])=>{quotes['USD/'+ccy]={symbol:'USD/'+ccy,type:'forex',price:+rate,open:+rate}});
  renderMarkets();renderTicker();
  const list=$('#marketList');if(!list)return;
  const fx=document.createElement('div');fx.className='card';fx.style.margin='12px 0 0';
  fx.innerHTML='<div class="row"><b>Global FX</b><span class="badge">Live spot</span></div>'+Object.entries(rates).map(([c,r])=>'<div class="v24-forex"><div class="v24-flag">'+c+'</div><div><b>USD/'+c+'</b><div class="small muted">1 USD</div></div><div class="price">'+Number(r).toFixed(c==='JPY'?2:4)+'<div class="change muted">spot</div></div></div>').join('');

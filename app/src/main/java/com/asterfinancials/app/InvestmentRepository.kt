...
class InvestmentRepository(private val context:Context){
    suspend fun list():Result<List<Investment>> = withContext(Dispatchers.IO){
        try{
            ...
            con.disconnect()
            if(code !in 200..299)return@withContext Result.failure(IllegalStateException("Investments unavailable."))
            val arr=JSONObject(text).optJSONArray("investments") ?: org.json.JSONArray()
            val out=mutableListOf<Investment>()
            for(i in 0 until arr.length()){
                val o=arr.getJSONObject(i)
                out.add(Investment(o.getString("id"),o.getString("category"),o.getDouble("principal"),o.getDouble("currentValue"),o.optDouble("projectionRate",0.0),o.optBoolean("compounding",true),o.getString("status")))
            }
            Result.success(out)

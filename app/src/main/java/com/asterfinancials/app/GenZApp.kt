    ...
    MaterialTheme(colorScheme=if(dark)darkColorScheme(background=bg,surface=Color.White.copy(alpha=.055f),primary=a,onPrimary=Color(0xFF14112A))else lightColorScheme(background=bg,surface=Color.White.copy(alpha=.78f),primary=a,onPrimary=Color(0xFF14112A))){
        Box(Modifier.fillMaxSize().background(bg)){
            ...
            Column(Modifier.fillMaxSize()){
                GenTopBar(dark,a,a2,{dark=!dark},{overlay="demo"})
...
@Composable private fun GenLab(a:Color,a2:Color,text:Color,mute:Color){
    ...
    LazyColumn(contentPadding=PaddingValues(16.dp,4.dp,16.dp,125.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
        ...
        item{GlassCard(a,a2,false){LabSlider("Starting amount",amount,100f,10000f,a){amount=it};LabSlider("Weekly projection %",rate,0f,30f,a){rate=it};LabSlider("Weeks",weeks,1f,52f,a){weeks=it};Text("ILLUSTRATIVE END VALUE",color=mute,fontSize=9.sp,modifier=Modifier.padding(top=8.dp));Text(fmt(result),fontSize=43.sp,fontWeight=FontWeight.Black,color=a);Text("Principal "+fmt(amount)+" + projected gain "+fmt(result-amount),color=mute,fontSize=10.sp);Text("This calculator does not predict or guarantee investment performance.",color=mute,fontSize=9.sp,modifier=Modifier.padding(top=8.dp))}}
    }
}
...
@Composable private fun MiniSparkline(c:Color){Canvas(Modifier.fillMaxWidth().height(76.dp)){val pts=(0..24).map{i->Offset(size.width*i/24f,size.height*(.45f-.28f*sin(i/3f)-.04f*sin(i*.9f)))};val p=Path().apply{pts.forEachIndexed{i,x->if(i==0)moveTo(x.x,x.y)else lineTo(x.x,x.y)}};drawPath(p,c,style=Stroke(4f,cap=StrokeCap.Round))}}
private fun fmt(v:Double,d:Int=2)=NumberFormat.getNumberInstance(Locale.US).apply{minimumFractionDigits=d;maximumFractionDigits=d}.format(v)
